...
```{python}
country_continents = country_continents.rename(columns={'country.name.en': 'country'})

pd.merge (tb_cases_children, country_continents, how = "left")
```
# inner join
...
```{python}
# keeps all information in the two datasets
