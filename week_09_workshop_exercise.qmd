...
```{python}
#population_total = population.loc [2]
tot_pop = population[population['indicator'] == 'SP.POP.TOTL']

tot_pop = population.query("indicator == 'SP.POP.TOTL'")

# melting the data to a long format
tot_pop_long = tot_pop.melt(id_vars= ['country', 'indicator'], var_name= 'year', value_name= 'total_population')

tot_pop_long = tot_pop_long.rename(columns={'country': 'country_code'})

# drop indicator columns
tot_pop_long = tot_pop_long.drop(columns= 'indicator')
tot_pop_long
...
hiv_data_combined_pop = pd.merge(hiv_data_combined, tot_pop_long, on=['country_code', 'year'], how= 'left')
...
hiv_data_combined_pop ['incidence_rate'] = (hiv_data_combined_pop['new_cases']/hiv_data_combined_pop['total_population'])*100000

hiv_data_combined_pop ['prevalence_rate'] = (hiv_data_combined_pop['total_cases']/hiv_data_combined_pop['total_population'])*100000
...
```{python}
hiv_data_mini = hiv_data_combined_pop[hiv_data_combined_pop['country'].isin(["Bangladesh", "Angola", "Belize"])]
...
fig = px.line(
    ...
    template='plotly_dark'  # Use a dark theme (you can change it)
)

# Customize appearance
fig.update_traces(line=dict(width=3, color='cyan'), marker=dict(size=8, color='red'))
...
fig.update_layout(
    ...
    xaxis=dict(showgrid=False, tickmode='linear', dtick=1),  # Ensure all years are visible
...
fig = px.line(
    ...
    y='prevalence_rate', 
    markers=True, 
    title='HIV Prevalence Rate Over Time',
    line_shape='spline',  # Smooth the line
    template='plotly_dark'  # Use a dark theme (you can change it)
)
