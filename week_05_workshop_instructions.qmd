...
```{python}
# Your code here
loans = pd.read_csv('D:\APHRC\Graph Courses_python\data\loans_full_schema.csv')
loans # 10,000 observations
```
...
credit_vars
#Exporting the subset to CSV file
...
prime.debt_to_income.value_counts() #debt to income is less than 20
prime.grade.value_counts() # confirm grades are A or B
prime.delinq_2y.value_counts() # delinquence is under 2 years
...
```{python}
# | eval: false
# pip install openpyxl --upgrade //installed using the terminal
...
with pd.ExcelWriter('outputs/loans_subsets.xlsx') as writer:
    ...
    demog_loan_info.to_excel(writer, sheet_name='Loan Information', index=False)
