...
pa["sedentary_ap_s_day"].value_counts()
pa

# Drop the 'seconds' variables
...
pa['total_pa_cat'] = pa['total_pa_m_wk'].apply(categorize)
...
```{python}
# Histogram of total minutes per week
fig = px.histogram(pa, x='total_pa_m_wk', nbins=5, title="Total physical activity per week")
fig.show()
```
...
```{python}
fig = px.box(pa, x='sex', y='total_pa_m_wk', title="BTotal physical activity by Gender")
fig.show()

# in general, males recorded more total minutes for physical activity compared to females
```
